...
`ec2-s3-inline-policy.json` is already scoped to the real bucket, `inventory-system-grp-2026` (region `eu-north-1`, per `server/.env`). If the bucket is ever renamed or recreated, update the ARN in that file to match.
...
4. Open the role → Add permissions → Create inline policy → JSON tab → paste the contents of `ec2-s3-inline-policy.json` → name it `ProductImageAccess` → Create policy.
